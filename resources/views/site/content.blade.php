                        ...
                        <div class="inner_section">
                            <div class="row">
                                <div class=" col-lg-4 col-md-4 col-sm-4 col-xs-12 pull-right">
                                   {!! Html::image('assets/img/'.$page->images,'ALT',['class' => 'img-circle delay-03s animated wow zoomIn']) !!}
                                </div>
                  ...
                  @endif
                    <div class="col-lg-4 {{($k % 3 ) > 0 ? 'borderLeft' : ''}} {{($k > 2)?'mrgTop':''}}">
                        <div class="service_block">
                            <div class="service_icon icon2  delay-03s animated wow zoomIn"> <span><i class="fa {{$item->icon}}"></i></span> </div>
                            <h3 class="animated fadeInUp wow">{{$item->name}}</h3>
                            <p class="animated fadeInDown wow">{{$item->description}}</p>
                        </div>
                    </div>
                    @if(($k+1) % 3 == 0)
                        </div>
...
@if(isset($portfolio) && is_object($portfolio))
        ...
        @foreach($portfolio as $item)
            ...
            <div style="position: absolute; left: 0px; top: 0px; transform: translate3d(0px, 0px, 0px) scale3d(1, 1, 1); width: 337px; opacity: 1;" class="portfolio-item one-four  {{$item->filter}} isotope-item">
                <div class="portfolio_img"> {{Html::image('assets/img/'.$item->images)}}</div>
...
<section class="page_section team" id="team"><!--main-section team-start-->
    <div class="container">
        ...
        <div class="team_section clearfix">
            ...
            @foreach($team as $k => $men)
            <div class="team_area">
                <div class="team_box wow fadeInDown delay-0{{$k * 3 + 2}}s">
                    <div class="team_box_shadow"><a href="javascript:void(0)"></a></div>
                   {{Html::image('assets/img/'.$men->images)}}
                    <ul>
                ...
                <div class="col-lg-8 wow fadeInLeft delay-06s">
                    <div class="form">
                        ...
                        <input class="input-btn" type="submit" value="send message">
                            <input type="hidden" class="input-btn" name="_token" value="{{csrf_token()}}">
                            {{csrf_field()}}
                        </form>
                    </div>
                </div>
